...
每当你收到一条消息，`setMessages()`就会使该组件重新渲染一个新的`message`数组，其中包括收到的消息。然而是，由于该`Effect`现在依赖于`messages`，这 也将 重新同步该`Effect`。所以每条新消息都会使聊天重新连接。用户不会喜欢这样！
...
用跟具体的`roomId`和`serverUrl` props 替换对象`options` props 会更好：
