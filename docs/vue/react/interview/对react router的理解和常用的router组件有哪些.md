...
```
上述组件当接收到的路由参数`name`不等于`tom`的时候，
将会自动重定向到首页。

**switch**

`switch`组件的作用适用于当匹配到第一个组件的时候，
后面的组件就不应该继续匹配。
...
动态路由的概念指的是`路由`中的路径并`不`会固`定`。

例如将`path`在`Route`匹配时写成`/detail/:id`，
那么`/detail/abc`、`/detail/123`等可以匹配到该`Route`。
